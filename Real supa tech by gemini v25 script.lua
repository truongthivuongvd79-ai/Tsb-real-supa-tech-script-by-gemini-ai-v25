...
local function applyR6Korblox(char, enable)
    ...
    local kModel = char:FindFirstChild("TSB_Korblox_Model")
    if enable then
        rLeg.Transparency = 1
        if not kModel then
            kModel = Instance.new("Part")
            kModel.Name = "TSB_Korblox_Model"
            kModel.Size = Vector3.new(1, 2, 1)
            kModel.CanCollide = false
            kModel.Massless = true
            kModel.Transparency = 0
            
            local mesh = Instance.new("SpecialMesh")
            mesh.Name = "KorbloxMesh"
            mesh.MeshType = Enum.MeshType.FileMesh
            mesh.MeshId = "rbxassetid://139607718"
            mesh.TextureId = "rbxassetid://139607702"
...
c.Stepped:Connect(function()if g and e.Character then local a3=e.Character;local a9=a3:FindFirstChildOfClass("Humanoid")if a9 then a9.AutoRotate=true end;for aa,ab in ipairs(a3:GetChildren())do if ab:IsA("Folder")or ab:IsA("StringValue")or ab:IsA("BoolValue")then local ai=string.lower(ab.Name)if string.find(ai,"stun")or string.find(ai,"freeze")or string.find(ai,"slow")then ab:Destroy()end end end end end)
